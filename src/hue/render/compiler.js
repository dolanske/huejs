...
/**
 *
 * Modified version of DAB0mb's jsx runtime compiler.
 * Thank you so much for this tutorial.
 *
 * TODO: Rewrite to return the correct render function
 *
 * https://github.com/DAB0mB/jsx-runtime
 */
...
export const createTemplate = (splits, ...values) => {
  // Replace newlines from the template

  // splits = [splits.replace(/[\n\r\t\s]+/g, ' ').replaceAll('> <', '><')]

  const root = parseElement(splits.join(placeholder), values)

  return createElementRenderFunction(root)
}

const createElementRenderFunction = (node) => {
  if (node.type === types.value) {
    return node.value
  }

  const children = [...node.children.map(createElementRenderFunction)]

  console.log(children)

  console.log(h(node.tag, node.props.props, children))

  return h(node.tag, node.props.props, children)
}
...
const parseProps = (str, values) => {
  ...
  while (match) {
    ...
    value = value.join('=')
    value =
      value === placeholder ? values.shift() : value ? value.slice(1, -1) : true
